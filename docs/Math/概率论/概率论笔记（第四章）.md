...
设 $ F $ 是一分布函数，$\{F_n, n \geq 1\}$ 是一列分布函数。如果对 **$ F $ 的每个连续点** $ x \in \mathbb{R} $，都有 $ F_n(x) \to F(x) $，则称 $ F_n $ 弱收敛(weak converges)于 $ F $，记作 $ F_n \xrightarrow{w} F $.

> 分布函数逐点收敛得到的极限函数未必是分布函数，例如
> $$
> F_n(x)=\cases{0,&x<n,\\1,&x≥n,}
> $$
> 对任意的$x \in R，F_{n}(x)\to 0$，但极限函数$G(x)$恒等于0，不是分布函数，因为分布函数满足$F(\infty)=1,F(-\infty)=0$
...
设 $\{F_n, n \geq 1\}$ 是一列分布函数，那么存在一个单调不减右连续的函数 $F$ (不一定是分布函数)，$0 \leq F(x) \leq 1$ ($x \in \mathbb{R}$)，和一子列 $\{F_{n_k}, k \geq 1\}$，使得对 $F$ 的每个连续点 $x$，$F_{n_k}(x) \to F(x)$ ($k \to \infty$)。

**Helly第二定理（掌握第一条）**

(1) 设 $ F $ 是一**分布函数**，$\{F_n, n \geq 1\}$ 是一列**分布函数**，$ F_n \xrightarrow{w} F $. 如果 $ g(x) $ 是 $\mathbb{R}$ 上的**有界连续函数**，则
...
而在第三章中，我们已经知道特征函数和分布函数可以相互唯一确定，同样地，Levy连续型定理的逆命题同样成立
...
2. 设$\xi$是一随机变量，{$\xi_n,n\geq1$}是一列随机变量，$g(x)$是$R$上的        连续函数，如果$\xi_n \xrightarrow{d} \xi$，则$g(\xi_n)\xrightarrow{d}g(\xi)$

​	  **证明** 假设 $\xi$ 和 $\{\xi_n, n \geq 1\}$ 的分布函数分别为 $F$ 和 $\{F_n, n \geq 1\}$。若 $\xi_n \xrightarrow{d} \xi$，则 $F_n \xrightarrow{w} F$。由Helly第二定理知

$$
\int_{-\infty}^{\infty} e^{itg(x)} \, \mathrm{d}F_n(x) \to \int_{-\infty}^{\infty} e^{itg(x)} \, \mathrm{d}F(x), \quad \forall t \in \mathbb{R}.
$$

​	上式将$e^{itg(x)}$视作Helly第二定理要求的有界连续函数，由逆极限定理可知 $g(\xi_n)$ 的分布函数弱收敛于 $g(\xi)$ 的分布函数（上式是特征函数逐点收敛），即可得到随机变量弱收敛
...
​	**证明**  令 $\varepsilon > 0$ 使得 $F$ 在 $ax + b \pm \varepsilon$ 处连续（因为 $F$ 的连续点在 $\mathbb{R}$ 上是稠密的）。显然 $a_n x + b_n \to ax + b$，因此对充分大的 $n$ 有
$$
...
$$
\mathbb{P}(a_n \xi_n + b_n \leq x) = F_n\left(\frac{x - b_n}{a_n}\right) \to F\left(\frac{x - b}{a}\right) = \mathbb{P}(a \xi + b \leq x).
$$
...
考虑一个n重伯努利试验，每次试验成功的概率为$p(0<p<1)$，记$S_n$为n重伯努利试验中成功的总次数，它是$n$个相互独立的$0-1(p)$随机变量的和，显然有
$$
S_n \sim B(n,p)
$$
人们往往对下列的概率大小感兴趣
...
> 事实上，$\frac{S_n-np}{\sqrt{npq}}$是$S_n$的标准化随机变量，棣莫弗-拉普拉斯定理告诉我们，当n充分大时，可以将$\frac{S_n-np}{\sqrt{npq}}$近似为标准正态随机变量$N(0,1)$
...
> 棣莫弗-拉普拉斯定理与我们第二章中提到的二项分布的泊松逼近定理并无矛盾，在泊松逼近定理中，我们要求$np_n \to \lambda$，而棣莫弗-拉普拉斯定理中要求$p$是常数，在实际应用中，当n很大时，若p大小适中，我们就用正态分布逼近二项分布；若p接近0或者1，且np较小或较大，此时二项分布的图像偏斜度太大，宜用泊松分布去逼近二项分布
>
> 并且在实际计算中，如果n不是很大，我们通常会将上式修正为
> $$
> \Phi(\frac{\beta+0.5-np}{\sqrt{npq}})-\Phi(\frac{\alpha-0.5-np}{\sqrt{npq}})
> $$
> 因为由于$\alpha,\beta$是非负整数
> $$
> P(\alpha\leq S_n \leq \beta)\approx\Phi(\frac{\beta-np}{\sqrt{npq}})-\Phi(\frac{\alpha-np}{\sqrt{npq}})\\
> P(\alpha\leq S_n \leq \beta)=P(\alpha-1<S_n<\beta+1)=\Phi(\frac{\beta+1-np}{\sqrt{npq}})-\Phi(\frac{\alpha-1-np}{\sqrt{npq}})
> $$
> 因此折中一下，加减0.5是合理的
...
**解** 令n为抛掷次数，$S_n$为出现正面的次数，则$S_n \sim B(n,0.5)$，由题意，需要求n使得
$$
P(0.4<S_n/n\leq0.6)\geq0.9
$$
且$ES_n=0.5n,VarS_n=n/4$，利用棣莫弗-拉普拉斯定理则有
$$
P(0.4<S_n/n\leq0.6)=P(\frac{0.4n-0.5n}{\sqrt{n/4}}<\frac{S_n-0.5n}{\sqrt{n/4}}\leq\frac{0.6n-0.5n}{\sqrt{n/4}})\approx\Phi(0.2\sqrt{n})-\Phi(-0.2\sqrt{n})\\=2\Phi(0.2\sqrt{n})-1
$$
那么现在问题即转化为求n使得
$$
2\Phi(0.2\sqrt{n})-1\geq0.9
$$
我们知道0.95介于$\Phi(1.64),\Phi(1.65)$之间，因此我们认为$\Phi(1.645)=0.95$，易知当$n\geq69$时满足要求

> 注意理解出现正面的频率在(0.4,0.6]之间的概率不小于90%，怎么把它转化为某个事件的概率？
...
$$
又因为$E\xi_1=a,Var\xi_1=\sigma^2$，所以$f(t)$有二阶连续导数且有下列的Taylor展开式
$$
...
并且 $S_{16} = \sum_{k=1}^{16} \xi_k$ 是 $n = 16$ 台彩电的辐射量。题目要求 $\mathrm{P}(S_{16} > 0.5)$。认为 $\{\xi_k, k \geq 1\}$ 是 i.i.d. 的，由 Lindeberg–Lévy CLT 知：
...
$$
\frac{S_n - \mathbb{E} S_n}{\sqrt{\operatorname{Var} S_n}} = \frac{\sum_{k=1}^n \xi_k - \mathbb{E} \left( \sum_{k=1}^n \xi_k \right)}{\sqrt{\sum_{k=1}^n \operatorname{Var} \xi_k}} \xrightarrow{d} N(0, 1).
$$
...
> $\xi_n \xrightarrow{P}\xi$可以直观地理解为：除去极小的可能性，只要n充分大，$\xi$和$\xi_n$的取值可以任意接近；并且**依分布收敛不能推出依概率收敛**
...
- 如果$\xi_n \xrightarrow{d}c$，c是常数，则$\xi_n \xrightarrow{P}c$
...
$$
这里由$\left\{\xi\leq x-\varepsilon,\xi_n >x\right\}$放大到{$\xi_n-\xi \geq \varepsilon$}
...
$$
\begin{aligned}
\{\xi_n \leq x\} &= \{\xi_n \leq x, \xi \leq x + \varepsilon\} \cup \{\xi_n \leq x, \xi > x + \varepsilon\} \\
&\subset \{\xi \leq x + \varepsilon\} \cup \{\xi - \xi_n \geq \varepsilon\},
\end{aligned}
$$
...
> 这里本来对于c的分布函数来说是有$x\leq c$的，但是$x=c$是不连续点，我们将其剔除，才符合分布函数弱收敛的定义
...
> 这是因为$c+\varepsilon-0>c$，分布函数在这点的值趋向于1，而$c-\varepsilon<c$，分布函数在这点的值趋向于0
...
\end{cases}
$$
而后者正是在a点退化的分布函数，于是我们得到了分布函数弱收敛，自然有随机变量依分布收敛到常数，从而得到依概率收敛
...
$$
\left\{|\xi-\eta|\geq\varepsilon \right\}=\left\{|\xi-\xi_n+\xi_n-\eta|\geq \varepsilon\right\} \sub \left\{|\xi_n-\xi|\geq \varepsilon/2\right\}\bigcup\left\{|\xi_n-\eta| \geq \varepsilon/2\right\}
$$
这里有一个概率论中很常用的手法，如果有事件A，B满足$A\sub B$，那么对于它们的概率就会有$P(A) \leq P(B)$
...
对**第二条**，首先因为$g(x)$在R上连续，所以其在R上的任意一个闭区间上一致连续。那么就会有对任意的$M>0$，$g(x)$在$[-M,M]$上一致连续，即有当$|x|\leq M,|y| \leq M$时，对给定的$\varepsilon>0$，存在$\delta>0$使得当$|x-y|<\delta$时，$|g(x)-g(y)|<\varepsilon$

所以有
$$
\begin{aligned}
\mathbb{P}(|g(\xi_n) - g(\xi)| \geq \varepsilon) & \\
\leq & \mathbb{P}(|g(\xi_n) - g(\xi)| \geq \varepsilon, |\xi_n| \leq M, |\xi| \leq M) \\
& + \mathbb{P}(|\xi_n| > M) + \mathbb{P}(|\xi| > M) \\
\leq & \mathbb{P}(|\xi_n - \xi| \geq \delta, |\xi_n| \leq M, |\xi| \leq M) \\
& + \mathbb{P}(|\xi_n - \xi + \xi| > M) + \mathbb{P}(|\xi| > M) \\
\leq & \mathbb{P}(|\xi_n - \xi| \geq \delta) + \mathbb{P}(|\xi_n - \xi| > M/2) + \mathbb{P}(|\xi| > M/2) \\
& + \mathbb{P}(|\xi| > M). 
\end{aligned}
$$
...
- 若$\xi_n \xrightarrow{P} \xi, \eta_n \xrightarrow{P}c$，$c$为一常数，假设$\eta_n$与$c$都不为0，则有$\frac{\xi_n}{\eta_n} \xrightarrow{P} \frac{\xi}{\eta}$
...
$$
\xi_n \xrightarrow{P} \xi当且仅当 E\frac{|\xi_n-\xi|^2}{1+|\xi_n-\xi|^2}\to 0
$$
**证明** 充分性：我们取$f(x)=\frac{x^2}{1+x^2}$，这个函数在$[0,\infty)$上是非负单调不减的函数，所以对任意的$\varepsilon>0$，由Markov不等式可以知道
$$
...
> 若把条件中的$\frac{1}{n^2}\sum_{k=1}^n \sigma_k^2 \to 0$改成$\frac{1}{n^2}Var(\sum_{k=1}^n \xi_k)$，此时不需要假设{$\xi_n,n \geq 1$}是独立随机变量序列，则定理的结论仍然成立，成为Markov大数定律
...
$$
**例2** （矩估计的相合性）假定总体$\xi$的一阶矩（即数学期望）$m_1$存在（隐含$E|\xi<\infty|$）但未知，通常的做法是对$\xi$进行n次独立重复观测，得到样本$\xi_1,\cdot\cdot\cdot,\xi_n$，并以它们的算术平均（也称为样本的1阶矩）$A_1=\frac{1}{n}\sum_{j=1}^n \xi_j$作为$m_1$的估计量，这种做法的依据就是辛钦大数定律
$$
A_1 \xrightarrow{P}m_1
$$
更重要的是，根据辛钦大数定律，若总体的k阶矩$m_k=E\xi^k$存在（隐含$E|\xi|^k<\infty$），这时样本的$k$阶矩$A_k =\frac{1}{n}\sum_{j=1}^n \xi_j^k$可作为$m_k$的估计量，依据是
$$
