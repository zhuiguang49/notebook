...
$$ X_n(\omega) =  \begin{cases}  (-1)^n n, & \text{当 } \omega \in (0, 1/n) \text{时}, \\ 0, & \text{当 } \omega \in [1/n, 1) \text{时}, \end{cases} $$ 
...
$$
也就是说，在满足 Fubini 定理的情形下，期望和积分能够交换顺序，下面我们看几道例题：
...
$$
**证明**：我们希望能够利用 Fubini 定理，此处已经出现了期望的形式，那么自然地，我们会想到去构造积分的形式，也就是将 $X^{\alpha}$ 变成积分的形式，容易想到
...
$$
   E(X1_A)=E(X|A)P(A)
$$
   这个性质是好理解的，$1_A$ 是示性函数，当 $A$ 发生时取 $1$ ，当 $A$ 不发生时取 $0$，
...
这里我们说明一下，题目中指的** $k$ 次相继出现正面**指的是连续 $k$ 次出现正面。做这题也有两个思考角度：
...
第二种：**递归方法**。要求相继 $k$ 次出现正面，必然需要相继 $k-1$ 次出现正面，所以我们可以去考虑 $E(N_k|N_{k-1})$，容易想到在已有 $N_{k-1}$ 的情况下，下一次抛硬币有两种情形，第一种是正面，这时 $E(N_k|N_{k-1})=E(N_{k-1})+1$；但是如若下一次抛硬币是反面，则又只能重新开始，这时会有 $E(N_k|N_{k-1})=E(N_{k-1})+1+E(N_k)$，那么由全期望公式，就有
$$
...
- 如果 $Y=h(X)$ 是 $X$ 的可测函数，则 $E(Y|X)=Y$，这是因为 $Y$ 本身是 $X$ 的函数， $X$ 确定之时， $Y$ 也确定了为一个定值，所以其期望是其本身
