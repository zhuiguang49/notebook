...
这里的逻辑是 $P(X=Y)=1\iff P(X-Y=0)=1\iff E[X-Y]^2=0$，所以就是要证明 $E[X(t+T_0)-X(t)]^2=0$，又
...
!!! tip "说明"
    ...
    下面内容部分参考了 @CC98 kausiujik 前辈的笔记
...
**例** 证明：余弦波 $X(t)=A\cos(\omega t+\theta),-\infty <t <\infty$，其中 $\omega$ 是常数，$A$ 与 $\theta$ 相互独立，

$$A\sim f(x)=\begin{cases}2x,&0<x<1\\0,&其他  \end{cases}$$

$\theta$ 在 $(0,2\pi)$ 上均匀分布，是宽平稳过程，并判断其是否为均值各态历经过程
...
$$\begin{aligned}
R_X(t_1,t_2)=E[X(t_1)X(t_2)]=E[A^2\cos(\omega t_1+\theta)\cos(\omega t_2+\theta)]\\=E[A^2]E[\cos(\omega t_1+\theta)\cos(\omega t_2+\theta)]=\frac{1}{4}\cos \omega(t_2-t_1)
\end{aligned}$$
...
**例** （Ornstein-Uhlenbeck 过程）设 $X(t)=e^{-\frac{\alpha t}{2}}B(e^{\alpha t})$，这里 $\alpha >0,\left\{B(t);t\geq 0\right\}$ 是标准布朗运动
...
**例** 设 $\left\{X_n;n\geq 0\right\}$ 是一个时齐的遍历的 Markov 链，状态空间 $I$ 有限，$f$ 是 $I$ 上的函数，设 $X_0$ 的分布是平稳分布 $\pi$，令 $Y_n=f(X_n)$
...
(3) 当 $N\to \infty$ 时，$\frac{1}{N}\sum_{i=1}^NY_i$ 依概率收敛吗？如果收敛，收敛到什么？说明理由
